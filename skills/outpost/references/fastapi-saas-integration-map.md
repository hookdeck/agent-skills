...
**Do not paste these files into another repository.** Reproduce the same **behavior** (env vars, `outpost.py` BFF patterns, domain publish via `httpx` to `POST …/publish`) in the user’s FastAPI layout and settings system. For FastAPI, `httpx`, and `outpost_sdk` **1.x** pins, see [SKILL.md](../SKILL.md#reference-example-applications) and [examples/fastapi-saas/backend/pyproject.toml](../examples/fastapi-saas/backend/pyproject.toml).
